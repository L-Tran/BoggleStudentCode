import java.util.ArrayList;
import java.util.Arrays;

// Completed by Logan Tran
public class Boggle {

    private static TST dict;
    private static boolean[][] isVisited;
    private static int boardSize;

    public static String[] findWords(char[][] board, String[] dictionary) {

        ArrayList<String> goodWords = new ArrayList<String>();
        boardSize = board.length;

        // Initialize array to indicate visited squares
        isVisited = new boolean[boardSize][boardSize];

        // Create TST for dictionary
        dict = new TST();
        for (String word: dictionary) {
            dict.insert(word);
        }

        // DFS for each square
        for(int i = 0; i < boardSize; i++) {
            for(int j = 0; j < boardSize; j++) {
                // Execute Depth first search
                dfs(i, j, "", board, goodWords);

            }
        }

        // Convert the list into a sorted array of strings, then return the array.
        String[] sol = new String[goodWords.size()];
        goodWords.toArray(sol);
        Arrays.sort(sol);
        return sol;
    }

    public static void dfs(int row, int col, String word, char[][] board, ArrayList<String> goodWords) {

        // Base cases for: outside squares and if square is visited
        if (row < 0 || col < 0 || row >= boardSize || col >= boardSize) {
            return;
        }
        if(isVisited[row][col]) {
            return;
        }

        // Add next square letter
        word += board[row][col];

        // If the new word is not a valid prefix
        if(dict.search(word) == null) {
            return;
        }

        // Check if prefix is a word
        if(dict.search(word).isWord()) {
            // Add to words and set word in TST to false in order to prevent duplicates
            goodWords.add(word);
            dict.search(word).setWord(false);
        }

        // Set visited to true when going through square
        isVisited[row][col] = true;

        // DFS all possible directions
        dfs(row + 1, col, word, board, goodWords);
        dfs(row, col + 1, word, board, goodWords);
        dfs(row - 1, col, word, board, goodWords);
        dfs(row, col - 1, word, board, goodWords);

        // Set visited to false after tracing path down to prepare for next
        isVisited[row][col] = false;
    }
}
